...
void loop() {
  ...
  uint8_t color = analogRead(COLOR_PIN) >> 2; 
  
  Serial.println(analogRead(BRIGHT_PIN), analogRead(COLOR_PIN) );    // Для этого удаляем младшие 2 бита
  ...
  //Serial.println(r_pwm, g_pwm, b_pwm);  
